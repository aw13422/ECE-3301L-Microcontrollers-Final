...
void delay_ms(int ms)                       // Function to delay ms
{
           
#define COUNT_PER_MS    _XTAL_FREQ/4000     // Count per ms = 2000             
#define COUNT_SCALED    COUNT_PER_MS/8      // Define COUNT_SCALED as COUNT_PER_MS/8
...
const char font[] =                         // array to store fonts
{
...
const char font2[] =                        // Array to store font2
{
...
{
    TFT_CS = 0;                             // Set TFT_CS to 0

    TFT_DC = 0;                             // Set TFT_DC to 0
    spiwrite(cmd_);                         // Call spiwrite() function
 
    TFT_CS = 1;                             // Set TFT_CS to 1
...
{
    TFT_CS = 0;                             // Set TFT_CS to 0
    TFT_DC = 1;                             // Set TFT_DC to 1
    spiwrite(data_);                        // Call spiwrite() function
    TFT_CS = 1;                             // Set TFT_CS to 1
}

void LCD_Reset(void)                        // Reset LCD screen
{
    TFT_RST = 0;                            // Clear TFT_RST to 0
    delay_ms(100);                          // Delay 100 ms
    TFT_RST = 1;                            // Set TFT_RST to 1
}
    
void Bcmd()                                 // Function for Bcmd
{
